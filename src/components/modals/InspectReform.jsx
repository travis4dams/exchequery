      ...
      {projection && Object.keys(projection).length > 0 && (
        <div className="mb-3">
          ...
          <div className="space-y-1 text-[11px] font-mono">
            ...
            {projection.debt && <div><span className="text-stone-400">Debt shock:</span> <span className={`tabular-nums ${projection.debt.value > 0 ? 'text-signal-bad' : 'text-signal-good'}`}>{projection.debt.value > 0 ? '+' : ''}£{projection.debt.low.toFixed(1)} to {projection.debt.value > 0 ? '+' : ''}£{projection.debt.high.toFixed(1)}bn one-shot</span> {reform.onComplete?.debt?.citationId && <CitationLink id={reform.onComplete.debt.citationId} className="ml-1" />}</div>}
            {projection.bondYield && <div><span className="text-stone-400">Bond yield:</span> <span className={`tabular-nums ${projection.bondYield.value > 0 ? 'text-signal-bad' : 'text-signal-good'}`}>{projection.bondYield.value > 0 ? '+' : ''}{projection.bondYield.low.toFixed(2)} to {projection.bondYield.value > 0 ? '+' : ''}{projection.bondYield.high.toFixed(2)}pp</span> {reform.onComplete?.bondYield?.citationId && <CitationLink id={reform.onComplete.bondYield.citationId} className="ml-1" />}</div>}
          </div>
        </div>
      )}
