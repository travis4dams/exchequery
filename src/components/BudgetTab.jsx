...
function pickRange(game, key) {
  ...
  return { min: v(block.min), max: v(block.max), unlocked };
}

function SectionCard({ eyebrow, meta, children }) {
  return (
...
export function BudgetTab({ game, committed, set }) {
  ...
  const taxExtremeOn   = r.taxIncomeBasic.unlocked;
  const spendExtremeOn = r.spendNHS.unlocked;

  return (
    <Stack gap="lg">
      <div>
        <h2 className="font-display text-2xl md:text-3xl font-medium italic text-stone-100 mb-1">Budget Levers</h2>
        <p className="text-[12px] text-stone-500">Revenue figures scale with GDP. Source IFS/HMRC ready-reckoner.</p>
      </div>

      {/* Revenue (Income / Corp+VAT) — two-column on desktop. */}
      <Grid cols={{ base: 1, md: 2 }} gap="md">
        <SectionCard eyebrow="Income Tax" meta={taxExtremeOn ? 'Extreme ranges unlocked' : null}>
        ...
        <SectionCard eyebrow="Corporation Tax & VAT" meta={taxExtremeOn ? 'Extreme ranges unlocked' : null}>
      ...
      <SectionCard eyebrow="Departmental Spending — Core" meta={spendExtremeOn ? 'Extreme ranges unlocked' : null}>
        <Grid cols={{ base: 1, md: 2 }} gap="md">
          <div>
      ...
      <SectionCard eyebrow="Other Departments">
