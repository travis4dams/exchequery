...
### New
- **Scottish Independence is now a reform.** State-branch, controversial — the heaviest lift in the game (PC 50, coalition 65, six quarters). Passing it permanently shaves rUK potential growth (−0.4 pp), adds a one-shot debt-share settlement (+£25bn), jolts bond yields (+0.4 pp), and shrinks the devolved-transfer floor from £65bn to £30bn so the player can finally cut Wales+NI block grants without spiking the Devolution-in-Revolt event. The event itself is largely suppressed after the reform passes. New `onComplete.debt` and `onComplete.bondYield` channels were added to the reform schema so future constitutional/structural reforms can deliver one-shot macro shocks at completion (distinct from the upfront `cost` that pays at commit).
- **Population now has a real breakdown.** Births, deaths and net migration are tracked as separate channels in thousands per quarter, each with its own driver line — population health lifts births and suppresses deaths, NHS funding pulls deaths down, labour-market slack reduces inflows. The Overview's Population panel surfaces the three numbers; click it to open a redesigned modal with sparklines and a one-line driver attribution per channel. Existing reform levers (immigrationCap, freeChildcare) now move the relevant channel instead of a single opaque growth figure.
- **Wages, jobs and education are now first-class.** A wage index (nominal + real-vs-CPI), an employment count (workforce × 1−unemployment), a productivity index, and an education index (0-100 with mean reversion to OECD-comparable 60) are tracked each quarter. Wage Phillips is asymmetric (hot-labour-market only), with productivity passthrough and an education premium feeding into wage growth. New Wages (real) and Jobs panels join the Wider Economy strip on Overview, and a Wages & Productivity panel pairs with Labour Market on the Markets tab. realLivingWage and unionRights now bump the wage index on completion; skillsBudget, uniReform and freeChildcare bump the education index.
- **Four new state-branch reforms with mutual-exclusion.** Open Migration Compact (open-up track, big labour-supply lift, hostile to working-class/northern blocs), Integration Package (the conciliatory middle path), Social Media Ban for under-16s, and Algorithmic Recommendation Ban (the bigger Haidt-flavoured demographic effect, prereq's the basic ban). The migration reforms form a track: passing one permanently blocks the others via a new `excludesComplete` schema field — both the picker and the engine commit gate respect it.
...
### Balance
- **Productivity now moves real GDP.** R&D, education and infrastructure improvements no longer stop at the wage block — the productivity-growth delta now also feeds the headline-growth mean-reversion anchor (using the lagged prior-quarter value to avoid a circular dep on Okun's unemployment). A sustained R&D bump that lifts productivity 0.4 pp/yr above trend now also pulls headline growth toward that same 0.4 pp/yr lift. Employment-driven growth still routes via the migration→GDP elasticity channel (no double-counting).
- **Hours-worked margin is now tracked.** Average weekly hours per worker (UK LFS ~32 h/wk) joins the labour-supply state. The labour-input metric scales employment by hours / hours-baseline, so future four-day-week or statutory-leave reforms can move effective labour without changing headcount.
- **Brexit handles exposed.** OBR's standing 4% long-run productivity drag and OMFIF's wage-Phillips steepening are no longer buried in `productivityTrend = 0.6%` and `phillipsCoef = 0.60` — they're separately tunable via `PARAMS.brexit.productivityDragPp` (default 0, additive) and `PARAMS.brexit.phillipsSlopeMultiplier` (default 1.0). Defaults are neutral so current behaviour is bit-identical; reforms can toggle them.
- **Fiscal drag from frozen thresholds.** Income-tax personal allowance and basic-rate threshold have been frozen at April-2021 nominal levels through 2027-28 (UK 2022 + 2022 Autumn Statement). The wage-bill portion of income tax now scales by a multiplicative drag factor that grows with the wage index: each 5% nominal wage growth lifts the IT-wage portion by an additional ~1.25%. Default ON (matches UK policy a 2026 chancellor inherits); a future reform could uprate.
- **Migration baseline rebased to OBR central.** Net migration now starts at 80k/quarter (≈320k/yr) — the midpoint of OBR's March 2025 central path (340k/yr) and the November 2025 downward revision (290k tapering to 260k near-term, recovering to 340k by 2030). Phase-1 preserved the legacy 17k/q baseline for bit-identity; that ceiling is gone. Labour-supply response to slack doubled in absolute terms (migrationUnempCoef -12 → -30) so a 2pp unemployment-NAIRU gap moves migration by ~75% of the new baseline — same design intent at a realistic scale.
- **NAIRU is now a state variable with hysteresis.** Initial natural rate raised 4.25% → 4.7% per BoE MPR Box F (November 2025), Resolution Foundation Q4 2025 LMO, and Pill 2025 speeches. NAIRU drifts toward live unemployment at 0.015/quarter (Ball 2009 hysteresis; ~46-quarter half-life, in line with Ball's "adjustment over multiple years" framing), bounded [3.5%, 6.5%]. Prolonged labour-market deterioration lifts the equilibrium upward and slowly back when conditions tighten.
- **Participation is now a state variable.** The 0.80 working-age participation rate is no longer a fixed PARAMS leaf. It drifts on healthIndex (LCWRA caseload channel — sustained NHS underspending lowers participation) and on freeChildcare completion (mothers' labour-force re-entry, +0.8pp), with mean reversion to the OECD long-run anchor. Reform strategies that route through public health now hit labour supply through both the unemployment and participation margins.
- **Wage Phillips coefficient raised 0.35 → 0.60.** Mid-point between Haldane's 2018 pre-Brexit reading (0.35) and Castle & Hendry's 2024 modern UK estimate (~0.84). Reflects the post-Brexit steepening evidence (Castle-Hendry 2023 CEPR VoxEU; OMFIF Dec 2025) without committing all the way to the steepest estimate. Hot labour markets now feed wages roughly 70% faster than before.
- **Productivity now responds to live policy levers.** updateProductivityIndex was a fixed-trend stub; it now blends a lagged AR(1) on prior-quarter annual growth (0.5 weight) with the OBR trend plus state-driven contributions from R&D spend (0.04pp/yr per £bn over baseline), educationIndex (0.03pp/yr per point over 60), and infrastructure (0.015pp/yr per £bn over baseline). The wage block's productivityPassthrough now transmits these driver-driven gains directly into wages.
- **Immigrant productivity gradient applied to migration→GDP elasticity.** Migration deltas now scale by 0.85 — the 5-year average over the Bell-Johnson (2023 MAC) / Hall-Manning (2024 CEP) immigrant earnings trajectory (0.75 initial → 0.95 after 5 years). The 20-quarter fiscal taper still provides the time spread; no per-cohort tracking required.

### Fixes
- **Social-media-ban birth coefficients retired.** The Twenge/Haidt-cited socialMediaBanBirthCoefQ (+2.5k/q) and socialMediaAlgoBanBirthCoefQ (+5k/q) births contributions are removed: the underlying literature supports a long-run cohort-fertility hypothesis (10-15yr lag), not a current-quarter response, and the peer-reviewed evidence is contested (Orben & Przybylski 2019; Coyne et al. 2020). The reforms themselves keep their healthBoost, bloc reactions, and equity-fintech effects — only the implausibly-fast births channel is gone.
- **HMRC wage-share citations upgraded sourced.** hmrc_wage_share_of_it and hmrc_ni_wage_base now cite HMRC Personal Incomes Statistics 2023-24 (published February 2026, "Employment income accounts for the largest proportion of total income at 70.8%") with the direct URL.
- **Wage-price spiral citation upgraded.** wage_spiral_judgement (the 4pp/yr trigger and 0.10 coefficient) is now extrapolated with citations to Castle-Doornik-Hendry-Martinez (2023) UK 150-year wage-price spiral threshold of 6-8% and Kohlscheen-Moessner (2022) ULC→CPI regime-conditional pass-through.
- **NHS-mortality channel gains BMJ Open anchor.** Marmot Review 2020 citation now cross-references Watkins et al. (2017) BMJ Open *Effects of health and social care spending constraints on mortality in England* — ~45,000 excess deaths 2011-2015 from spending shortfalls, broadly consistent with deathsNHSCoef = -0.4.

### Known Issues
- **obrCentralPath finalUnemployment and finalBankRate stay skipped.** Audit-informed NAIRU (4.7%) sits above OBR's central-case implicit NAIRU; audit-informed neutral rate (4.0%) sits above OBR's glide path. The sim's terminal unemployment (~5.17%) and Bank Rate (~4.11%) therefore land just outside the ±25% band around OBR's published 4.1% / 3.25% targets. This is a deliberate divergence: the benchmark is OBR's forecast (correct), the sim is post-audit literature (correct), and the gap is the live UK macro policy debate.
...
- **Secondary header strip rearranged.** Growth already appears under GDP in the hero, so it's been replaced by Health on the secondary strip. New order: Health · Gini · Inflation · Gilts · PC.
- **Markets and Budget redesigned to match.** Markets panels now use the full panel width for their trajectory line (no more thumbnail sparklines) and pair up two-across on desktop. Budget sliders gain a baseline tick mark and a brass-filled track so you can see at a glance where the default sits; Departmental Spending splits into Core and Other on a wider grid. The fiscal cards (Revenue / Spending / Balance / National Debt) live on Overview now — see above.
